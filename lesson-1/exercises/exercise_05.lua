...
-- Modify ex3 to succeed only if the entire input is captured

local numeral = lpeg.C(lpeg.S("+-") ^ 0 * lpeg.R("09") ^ 1)
local plusOperator = lpeg.Cp() * lpeg.P("+")
local space = lpeg.S(" \n\t") ^ 0

-- let's match one number first, then attempt a plus and next number until end.
local matcher = numeral * (space * plusOperator * space * numeral) ^ 0 * -1
...
function TestMatcher()
  ...
  local testInputsAndExpected = {
    ...
    -- new: fail if not whole subject captured
    { input = "8\n",                    expected = {} },
    { input = "3+12!",                  expected = {} },
